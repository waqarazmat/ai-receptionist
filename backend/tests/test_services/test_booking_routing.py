...
class TestShouldRouteToBooking:
    ...
    # ── the reported bug: questions/greetings in early states are NOT trapped ──
    def test_faq_in_collecting_service_is_answered_not_looped(self):
        # "which services do you offer?" while collecting the service
        assert should_route_to_booking(True, BookingState.COLLECTING_SERVICE, "faq") is False

    def test_greeting_in_collecting_service_breaks_out(self):
        assert should_route_to_booking(True, BookingState.COLLECTING_SERVICE, "greeting") is False

    def test_faq_in_collecting_time_is_answered(self):
        assert should_route_to_booking(True, BookingState.COLLECTING_TIME, "faq") is False

    def test_off_topic_in_collecting_service_breaks_out(self):
        assert should_route_to_booking(True, BookingState.COLLECTING_SERVICE, "off_topic") is False
